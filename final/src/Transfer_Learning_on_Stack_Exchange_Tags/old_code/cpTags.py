...
def getRare(tags_num ,threshold):
    rareTags =  [i[0] for i in b if int(i[1]) < threshold]
    return rareTags
 
 def flatten(list2d):
    return [j for i in list2d for j in i]

tags = [ all_tags[i].split("-") for i in range((len(all_tags))) ]
tags = np.array( tags )

# myTags = readmyTags(sys.argv[1])
myTags = readTxt(sys.argv[1])
myTags = getRare(b, 25)
myTags_split = np.array( [ myTags[i].split("-") for i in range((len(myTags))) ] )


bTags = jointLists( bigramTags(tags))
mTags = jointLists( monogramTags(tags))
tTags = jointLists( trigramTags(tags))

mybTags = jointLists(bigramTags(myTags_split))
mymTags = jointLists(monogramTags(myTags_split))
mytTags = jointLists(trigramTags(myTags_split))
'''
arr_match = matching(mybTags, bTags)
arr_acc  = np.add.accumulate(arr_match)

arr_match = matching(mymTags, mTags)
arr_acc  = np.add.accumulate(arr_match)
'''
rmTags, fmTags = matchingTF(mymTags, mTags)
rbTags, fbTags = matchingTF(mybTags, bTags)
rtTags, ftTags = matchingTF(mytTags, tTags)
print(len(fmTags),",",len(fbTags),",",len(ftTags))

# plotArr(arr_acc)

tags_match = np.array([ mybTags[i] + " " + str(arr_match[i]) for i in range(len(mybTags)) ]) 
